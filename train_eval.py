...
def main():
    p = argparse.ArgumentParser(description="Train and evaluate Adam, RITE, and PRISM LoRA adapters on Math-10K or GLUE8.")
    p.add_argument("--dataset", choices=["math10k", "math", "glue8", "glue"], required=True)
    p.add_argument("--method", choices=["adam", "rite", "prism"], required=True)
    p.add_argument("--privacy", choices=["dp", "nondp", "non-dp"], default="dp")
    p.add_argument("--epsilon", type=float, default=6.0, help="Target epsilon for DP runs.")
    ...
    p.add_argument("--steps", type=int, default=None, help="Override dataset default update steps.")
    p.add_argument("--batch_size", type=int, default=64)
    p.add_argument("--micro_batch_size", type=int, default=4)
    p.add_argument("--lr", type=float, default=None, help="Override dataset/method learning rate.")
    ...
    p.add_argument("--repeat_id", type=int, default=None, help="Optional suffix for repeated runs with the same seed.")
    ...
    p.add_argument("--fast_dev_run", type=int, default=0, help="GLUE eval debug limit; 0 means full validation.")
    args = p.parse_args()
    ...
    if cfg.run_eval:
        if cfg.dataset == "math10k":
            evaluate_math10k(
                cfg,
                batch_size=args.eval_batch_size or 64,
                num_beams=args.num_beams or 4,
                max_new_tokens=args.max_new_tokens or 256,
                max_input_length=args.max_input_length or 1024,
            )
        else:
            evaluate_glue8(
                cfg,
                batch_size=args.eval_batch_size or 128,
                num_beams=args.num_beams or 1,
                max_new_tokens=args.max_new_tokens or 8,
                max_input_length=args.max_input_length or 384,
