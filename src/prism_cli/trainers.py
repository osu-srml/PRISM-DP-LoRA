...
class RunConfig:
    ...
    def finalize(self) -> "RunConfig":
        ...
        if self.method not in {"adam", "rite", "prism"}:
            raise ValueError("method must be one of: adam, rite, prism")
        ...
        if self.dataset == "math10k":
            ...
            self.learning_rate = self.learning_rate or 3e-4
            ...
            self.learning_rate = self.learning_rate or 2e-4
...
def _make_private(privacy_engine, model, optimizer, train_loader, cfg: RunConfig, epochs_est: int):
    try:
        out = privacy_engine.make_private_with_epsilon(
            module=model,
            optimizer=optimizer,
...
def _make_loader(cfg: RunConfig, tokenizer):
    ...
    # Notebook-equivalence detail:
    # The Math-10K PRISM notebook used in the paper artifact did not pass an
    # explicit torch.Generator to DataLoader.  Because model construction and
    # spectral residual initialization consume torch RNG before the sampler is
    # instantiated, adding an explicit generator changes the minibatch order and
    # can noticeably change Math-10K utility.  GLUE8 notebooks used an explicit
    # generator, so we keep that path for GLUE8.  This preserves the command-line
    # behavior of the corresponding notebook artifacts while leaving all base
...
def _build_prism_optimizer(cfg: RunConfig, model):
    from .optim.prism import PRISM, get_paired_lora_parameters, spectral_init_peft_model

    print("[init] spectral residual initialization")
    spectral_init_peft_model(
        model,
        svd_device=cfg.spectral_svd_device,
        svd_oversample=int(cfg.spectral_oversample),
        svd_n_iter=int(cfg.spectral_n_iter),
        verbose=True,
    )
...
def _build_baseline_optimizer(cfg: RunConfig, model):
    if cfg.method == "adam":
        return torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=float(cfg.learning_rate))
    if cfg.method == "rite":
        from .optim.lora_rite import LoRARite

        params = [p for n, p in model.named_parameters() if ("lora" in n and p.requires_grad)]
        if not params:
            raise RuntimeError("No trainable LoRA params found for RITE optimizer.")
        return LoRARite(params, lr=float(cfg.learning_rate), betas=(0.9, 0.999))
...
def train_baseline_dp(cfg: RunConfig) -> Path:
    from opacus import PrivacyEngine
    ...
    set_seed(cfg.seed)
    model, tokenizer, device = _build_lora_model(cfg)
    base_optimizer = _build_baseline_optimizer(cfg, model)
    train_loader, train_ds = _make_loader(cfg, tokenizer)
    start_step = _restore_if_possible(cfg, model, base_optimizer)

    epochs_est = max(1, math.ceil(int(cfg.total_update_steps) / max(1, len(train_loader))))
    privacy_engine = PrivacyEngine()
    (model, optimizer, train_loader), used_mode = _make_private(privacy_engine, model, base_optimizer, train_loader, cfg, epochs_est)
    noise_multiplier = float(getattr(optimizer, "noise_multiplier", 0.0))
    sample_rate = float(getattr(optimizer, "sample_rate", float(cfg.batch_size) / float(len(train_ds))))
    for _ in range(start_step):
        _step_accountant(privacy_engine, noise_multiplier, sample_rate)
    print(f"[DP] grad_sample_mode={used_mode} noise_multiplier={noise_multiplier} sample_rate≈{sample_rate:.6g}")

    base_opt = getattr(optimizer, "original_optimizer", None) or getattr(optimizer, "_optimizer", None) or getattr(optimizer, "optimizer", None)
    if base_opt is None:
        raise RuntimeError("Cannot access wrapped base optimizer.")

    update_counter = {"n": int(start_step)}
    original_step = base_opt.step
    ...
    base_opt.step = counted_step
    Path(cfg.output_dir).mkdir(parents=True, exist_ok=True)
    logger = JsonlLogger(cfg.log_jsonl)
    model.train()
    pbar = tqdm(total=int(cfg.total_update_steps), initial=int(start_step), desc=f"{cfg.method}/dp updates")
    last_updates = int(start_step)

    while update_counter["n"] < int(cfg.total_update_steps):
        with BatchMemoryManager(data_loader=train_loader, max_physical_batch_size=int(cfg.micro_batch_size), optimizer=optimizer) as memory_safe_loader:
            for batch in memory_safe_loader:
                if update_counter["n"] >= int(cfg.total_update_steps):
                    break
                optimizer.zero_grad(set_to_none=True)
                batch = {k: v.to(device) for k, v in batch.items()}
                out = model(**batch)
                ...
                loss.backward()
                optimizer.step()
                if update_counter["n"] > last_updates:
                    pbar.update(update_counter["n"] - last_updates)
                    last_updates = update_counter["n"]
                    rec = {
                        "method": cfg.method,
                        "privacy": cfg.privacy,
                        "step": last_updates,
                    ...
                    try:
                        ...
                        pass
                    logger.log(rec)
                    if last_updates % 10 == 0 or last_updates == int(cfg.total_update_steps):
                        print(f"[{cfg.method}/dp] step={last_updates} loss={rec['loss_mean']:.4f}")
                    if cfg.resume and last_updates % int(cfg.checkpoint_every) == 0 and last_updates < int(cfg.total_update_steps):
                        save_resume_checkpoint(Path(cfg.output_dir), model, base_opt, last_updates)
                if update_counter["n"] >= int(cfg.total_update_steps):
                    break
    ...
    if checkpoint_file(Path(cfg.output_dir)).exists():
        checkpoint_file(Path(cfg.output_dir)).unlink()
    save_status(Path(cfg.output_dir), state="completed", method=cfg.method, privacy=cfg.privacy, update_steps=update_counter["n"])
    cleanup_cuda()
    print("Saved adapter to:", cfg.output_dir)
    return Path(cfg.output_dir)


def train_baseline_nondp(cfg: RunConfig) -> Path:
    clean_output_dir(Path(cfg.output_dir), cfg.force_train)
    if adapter_is_complete(Path(cfg.output_dir)) and not cfg.force_train:
        print("[train] completed adapter found; skipping:", cfg.output_dir)
        return Path(cfg.output_dir)
    sys.path.insert(0, str(llm_adapters_dir(cfg.root)))
    sys.path.insert(0, str(Path(__file__).resolve().parent / "optim"))
    finetune = importlib.import_module("finetune")
    optim_name = "lora_rite" if cfg.method == "rite" else "adamw"
    finetune.train(
        base_model=cfg.base_model,
        ...
        num_epochs=1,
        num_steps=int(cfg.total_update_steps),
        learning_rate=float(cfg.learning_rate),
        cutoff_len=int(cfg.cutoff_len),
        val_set_size=int(cfg.val_set_size),
        eval_step=int(cfg.eval_step),
        save_step=int(cfg.save_step),
        lora_r=int(cfg.lora_r),
        lora_alpha=int(cfg.lora_alpha),
        lora_dropout=float(cfg.lora_dropout),
        optim=optim_name,
    ...
    save_status(Path(cfg.output_dir), state="completed", method=cfg.method, privacy=cfg.privacy, update_steps=int(cfg.total_update_steps))
    cleanup_cuda()
    return Path(cfg.output_dir)
...
def train(cfg: RunConfig) -> Path:
    ...
    if cfg.privacy == "dp":
        return train_baseline_dp(cfg)
    return train_baseline_nondp(cfg)
