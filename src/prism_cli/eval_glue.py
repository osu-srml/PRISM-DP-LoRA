...
try:
    import torch
except Exception:  # imported lazily at runtime if unavailable during --help
    torch = None
...
def glue_to_instruction_input(task: str, ex: dict):
    ...
    if task == "cola":
        return (
            "Task: CoLA (linguistic acceptability). Determine whether the sentence is grammatically acceptable. Answer with exactly one label: acceptable, unacceptable.",
            f"Sentence: {ex['sentence']}",
        )
    if task == "sst2":
        return (
            "Task: SST-2 (sentiment). Determine the sentiment of the sentence. Answer with exactly one label: positive, negative.",
            f"Sentence: {ex['sentence']}",
        )
    if task == "mrpc":
        return (
            "Task: MRPC (paraphrase). Determine whether the two sentences are semantically equivalent. Answer with exactly one label: equivalent, not_equivalent.",
            f"Sentence 1: {ex['sentence1']}\nSentence 2: {ex['sentence2']}",
        )
    if task == "stsb":
        return (
            "Task: STS-B (semantic textual similarity). Rate the semantic similarity of the two sentences on a scale from 0 to 5 (0 = completely different, 5 = equivalent in meaning). Answer with a single number (you may use one decimal place).",
            f"Sentence 1: {ex['sentence1']}\nSentence 2: {ex['sentence2']}",
        )
    if task == "qqp":
        return (
            "Task: QQP (duplicate questions). Determine whether the two questions are duplicates. Answer with exactly one label: duplicate, not_duplicate.",
            f"Question 1: {ex.get('question1') or ''}\nQuestion 2: {ex.get('question2') or ''}",
        )
    if task == "mnli":
        return (
            "Task: MNLI (natural language inference). Given a premise and a hypothesis, determine the relationship. Answer with exactly one label: entailment, neutral, contradiction.",
            f"Premise: {ex['premise']}\nHypothesis: {ex['hypothesis']}",
        )
    if task == "qnli":
        return (
            "Task: QNLI (question-answering NLI). Given a question and a sentence, determine whether the sentence entails the answer to the question. Answer with exactly one label: entailment, not_entailment.",
            f"Question: {ex['question']}\nSentence: {ex['sentence']}",
        )
    if task == "rte":
        return (
            "Task: RTE (recognizing textual entailment). Given a premise and a hypothesis, determine whether the premise entails the hypothesis. Answer with exactly one label: entailment, not_entailment.",
